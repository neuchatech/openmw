...
void main()
{
...
#if PER_PIXEL_LIGHTING
    float shadowDistance = -passViewPos.z;
#else
    float shadowDistance = linearDepth;
#endif

    float shadowing = unshadowedLightRatio(linearDepth, shadowDistance);
    ...
    applyShadowDebugOverlay(shadowDistance);
}
